...
## 开发哲学：Human / Agent / CLI

AIOS 开发默认采用三层分工，而不是把所有智能、状态和执行都塞进 skill 或聊天：
...
- **Human** 负责方向、价值判断、审美取舍、风险授权和最终验收。
- **Agent** 负责理解意图、解释状态、设计选项、协调工具、发现缺口，并在需要时把真实决策交还给 Human。
- **CLI / script** 负责可重复、可校验、低歧义的结构动作，例如 init、append event、validate、doctor、render、index、archive、promote dry-run。

因此，skill 应保持薄入口和路由职责；重复结构逻辑应优先下沉为项目协议、CLI、脚本或 schema。Agent 面向用户的 UX 可以是自然语言，但落到机器层应尽量使用 dry-run、doctor/status/validate、JSON/status 输出和幂等命令。

## Workflow / LLL 语义分层

当 AIOS 引入更丰富的工作流概念时，先把它们作为 LLL 之上的语义层，而不是新建一套平行真源：

- LLL workdir 是可恢复、可审计的工作现场与 operational kernel。
- Matter / 事务可作为长期工作的默认聚合根；Project、Workflow Run、Case、Engagement、Issue、Kanban card、HTML report 更适合作为视图、subtype 或 projection。
- Markdown/HTML 报告、图表和 dashboard snapshot 是 Presentation Views，可以较早出现，用于压缩状态和提升决策质量。
- Kanban、GitHub、runner、CI、飞书审批等会改变状态的系统是 Execution Projections，只有在 link-back、权限边界、回写、冲突处理和验证路径清楚时才接入。
- Closeout 时应区分 deliverable、asset candidate、archived evidence 和 pruned noise；AIOS vault/项目文档/数字花园只接收精选语义资产，不接收原始过程垃圾。

这套分层应能在个人 AIOS、团队协作和企业数字化交付之间通过 extension、view、policy、adapter 展开，而不是为不同尺度重写核心协议。
